...
def _register_cache_serialization(verbose: int = 0) -> Dict[str, bool]:
    ...
    return dict(
        DynamicCache=unregistered_dynamic_cache, MambaCache=unregistered_mamba_cache
    )
...
def bypass_export_some_errors(
    ...
    * replaces :class:`transformers.cache_utils.DynamicCache` with ``patched_DynamicCache``
...
def replacement_before_exporting(args: Any) -> Any:
    ...
    :class:`transformers.cache_utils.DynamicCache` by ``patched_DynamicCache``
    """
    if args is None:
        return None
