...
torch_export(layer, x, "basic.torch.onnx", verbose=False, input_names=["X"], output_names=["Y"])
...
te_export(layer_te, x, "basic.te.onnx", input_names=["X"], output_names=["Y"], use_fp8=False)
print("to_onnx - torch - TransformerEngine - f8")
try:
    te_export(layer_te, x, "basic.te.fp8.onnx", input_names=["X"], output_names=["Y"], use_fp8=True)
...
torch_export(layer_norm, x, "norm.torch.onnx", verbose=False, input_names=["X"], output_names=["Y"])
...
te_export(te_layer_norm, x, "norm.te.onnx", input_names=["X"], output_names=["Y"], use_fp8=False)
te_export(te_layer_norm, x, "norm.te.fp8.onnx", input_names=["X"], output_names=["Y"], use_fp8=False)
    ...
    def tracefunc(frame, event, arg, indent=[0]):
        ...
        if event == "call":
            ...
            if global_in:
                ...
                print("-" * indent[0] + f"> {frame.f_code.co_name!r} - {filename}:{line}")
                indent[0] += 2
        elif event == "return":
            indent[0] -= 2
            if global_in:
