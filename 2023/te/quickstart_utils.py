...
class DotProductAttention(torch.nn.Module):
    ...
    def masked_softmax(
        self, inp: torch.Tensor, mask: Optional[torch.Tensor]
    ) -> torch.Tensor:
        if mask is not None:
    ...
    def forward(
        ...
        bmm1 = (
            torch.bmm(query.transpose(0, 1), key.transpose(0, 1).transpose(1, 2))
            / self.norm_factor
        )
...
def cast_to_representable(inp, scale=1.0, fp8_format="e4m3"):
    ...
    ret = texcpp.cast_from_fp8(
        ret, meta, tex.FP8FwdTensors.GEMM1_INPUT, fp8_type, input_type
    )
    return ret
