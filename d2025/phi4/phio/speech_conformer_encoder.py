...
class NemoConvSubsampling(torch.nn.Module):
    ...
    def forward(self, x, mask):
        ...
        padding_length = torch.ceil(feature_lens.to(torch.float32) / float(self.subsampling_factor)).to(torch.int64)
        ...
        condition = torch.full_like(pad_mask, batch_size != 1).bool()
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(
        ...
        """
        # self.h = num_heads
        # self.h_k = num_kv_heads
        # self.d_k = head_size
        n_batch, seq_len, _ = query.size()

        q = self.linear_q(query).view(n_batch, seq_len, self.h, self.d_k)  # (b, t, d)
        k = self.linear_k(key).view(n_batch, seq_len, self.h_k, self.d_k)  # (b, t, d)
        v = self.linear_v(value).view(n_batch, seq_len, self.h_k, self.d_k)
        q = (
            q.transpose(1, 2)
        ...
        if self.use_pt_scaled_dot_product_attention and not torch.jit.is_scripting():
            ...
            if pos_v is not None:
                ...
                x = x + attn_v
        x = (
            x.transpose(1, 2).contiguous().view(n_batch, seq_len, self.h_k * self.d_k)
        )  # (batch, time1, d_model)
...
class TransformerEncoderBase(abc.ABC, nn.Module):
    ...
    def compute_lens_change(self, feature_lens):
        ...
        if self.input_layer == "nemo_conv":
            ...
            return math.ceil(feature_lens / self.time_reduction)
    ...
    def _streaming_mask(self, seq_len, batch_size, chunk_size, left_chunk):
        ...
        if self.training and np.random.rand() > 0.5:
            ...
            chunk_start_idx = torch.tensor([], dtype=torch.int64)
    ...
    def expand_mask(self, mask):
        # Convert `mask = torch.tensor([])` to `mask = torch.tensor(0)`
        # and leave `mask` unmodified otherwise
        orig_num_elements = mask.numel()
        new_num_elements = max(1, orig_num_elements)
        mask_shape = list(mask.shape)
        mask_shape[0] = max(1, mask_shape[0])
        expanded_mask = torch.zeros(new_num_elements, dtype=mask.dtype)
        expanded_mask[ : orig_num_elements] = mask.flatten()
        expanded_mask = expanded_mask.view(mask_shape)
    ...
    def forward_embeddings(self, xs_pad, masks, chunk_size_nc=None, left_chunk_nc=None):
        ...
        # get new lens.
        seq_len = self.compute_lens_change(xs_pad.shape[1])
        if seq_len <= 0:
            raise ValueError(
        ...
        # Select correct `hs_mask`
        streaming_mask = enc_streaming_mask
        expanded_masks = self.expand_mask(masks)
        expanded_streaming_mask = self.expand_mask(streaming_mask)

        if_condition = torch.full_like(expanded_streaming_mask, streaming_mask.numel() > 0 and batch_size > 1).bool()
        elif_condition = torch.full_like(expanded_masks, streaming_mask.numel() == 0 and batch_size > 1).bool()
        else_condition = ~elif_condition
        hs_mask = (expanded_masks & expanded_streaming_mask) * if_condition + expanded_masks * elif_condition + expanded_streaming_mask * else_condition
...
class ConformerEncoder(TransformerEncoderBase):
    ...
    def __init__(  # pylint: disable-all
        ...
        for i in range(len(self.encoders)):
            self.encoders[i] = self.encoders[i]._checkpoint_wrapped_module
    ...
    def calculate_hs_mask(self, xs_pad, device, mask):
        ...
        if_condition = torch.full_like(enc_streaming_mask, batch_size == 1).bool()
        else_condition = ~if_condition
        return enc_streaming_mask * if_condition + pad_mask * else_condition
    ...
    def forward(self, xs_pad, masks):
        ...
        # Rewritten unfold logic
        ori_bz, seq_len, D = input_tensor.shape
        max_seq_len = 500 # maximum position for absolute positional encoding

        chunk_pad_size = (max_seq_len - (seq_len % max_seq_len)) % max_seq_len
        # the unfold op will drop residual frames, pad it to the multiple of max_seq_len
        if_padding_tensor = torch.zeros((ori_bz, chunk_pad_size, D), device=input_tensor.device)
        if_padded_input_tensor = torch.cat((input_tensor, if_padding_tensor), dim=1).to(input_tensor.device)

        new_bz = ori_bz * ((seq_len + chunk_pad_size) // max_seq_len)
        if_unfolded_tensor = if_padded_input_tensor.reshape(new_bz, max_seq_len, D)
        ...
        extra_padded_subsampled_padding_tensor = torch.zeros((ori_bz, chunk_pad_size), device=subsampled_pad_mask.device)
        extra_padded_subsampled_pad_mask = torch.cat((subsampled_pad_mask, extra_padded_subsampled_padding_tensor), dim=1).to(subsampled_pad_mask.device) # extra padding to the pad mask
        if_masks_unfold = extra_padded_subsampled_pad_mask.reshape(new_bz, max_seq_len) # unfold the pad mask like we did to the input tensor

        # Calculate values in masks_unfold to use
        # If condition is true, all values in masks_unfold are left as is.
        # If condition is false, all values in masks_unfold are set to 0.
        condition = torch.full_like(if_masks_unfold, ori_bz != 1).bool()
        masks_unfold = if_masks_unfold * condition

        if_hs_mask = self.calculate_hs_mask(if_unfolded_tensor, if_unfolded_tensor.device, masks_unfold)

        # Pad original hs_mask to be the same shape as if_hs_mask
        hs_mask_dim0, hs_mask_dim1, hs_mask_dim2 = hs_mask.shape
        if_hs_mask_dim0, if_hs_mask_dim1, if_hs_mask_dim2 = if_hs_mask.shape
        padded_dim0 = max(hs_mask_dim0, if_hs_mask_dim0)
        padded_dim1 = max(hs_mask_dim1, if_hs_mask_dim1)
        padded_dim2 = max(hs_mask_dim2, if_hs_mask_dim2)

        padded_hs_mask = torch.zeros(padded_dim0, padded_dim1, padded_dim2, device=input_tensor.device)
        padded_hs_mask[:hs_mask_dim0, :hs_mask_dim1, :hs_mask_dim2] = hs_mask
        padded_if_hs_mask = torch.zeros(padded_dim0, padded_dim1, padded_dim2, device=input_tensor.device)
        padded_if_hs_mask[:if_hs_mask_dim0, :if_hs_mask_dim1, :if_hs_mask_dim2] = if_hs_mask

        if_condition = torch.full_like(padded_if_hs_mask, seq_len > max_seq_len).bool()
        else_condition = ~if_condition
        chosen_padded_hs_mask = padded_if_hs_mask * if_condition + padded_hs_mask * else_condition

        # Remove any padding from chosen_padded_hs_mask
        shape_dim1, shape_dim2 = min(hs_mask_dim1, if_hs_mask_dim1), min(hs_mask_dim2, if_hs_mask_dim2)
        hs_mask = chosen_padded_hs_mask[:, :shape_dim1, :shape_dim2]

        # layer_emb = None
        hs_mask = hs_mask.to(torch.int32).unsqueeze(1).eq(0)  # (batch, 1, time1, time2)
        ...
        input_tensor = input_tensor[:, :seq_len, :]  # original code does :-chunk_pad_size to remove padding, which makes the tensor the same shape as before any padding
