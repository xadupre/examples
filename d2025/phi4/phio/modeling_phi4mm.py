...
def get_image_embeddings(
    image_features,          # [bs, max_num_crops, base_feat_height * base_feat_width (24*24), C]
    ...
    for i, img_size in enumerate(image_sizes):
        ...
        # Compute common constants used frequently that are dependent on values in for-loop
        ...
        glb_img = (
            ...
            .reshape(1, H_bfhr, bfhr, H_bfhr, bfhr, C).contiguous().permute(0, 1, 3, 2, 4, 5)
            .reshape(1, H_bfhr, H_bfhr, bfhr_2_C).contiguous()
        )
        ...
        sub_img = (
            ...
            .reshape(B_, H_bfhr, bfhr, H_bfhr, bfhr, C).contiguous().permute(0, 1, 3, 2, 4, 5)
            .reshape(B_, -1, bfhr_2_C).contiguous()
        )
        sub_img = (
            sub_img.reshape(1, h, w, bfh_bfhr, bfw_bfhr, -1).permute(0, 1, 3, 2, 4, 5)
        ...
        useful_height = int(reshaped_attention_mask[0, :, 0].sum().to(torch.int64).item())
        useful_width = int(reshaped_attention_mask[0, 0, :].sum().to(torch.int64).item())
...
@torch.jit.script_if_tracing
def get_merged_audio_set_tensor(audio_set_tensor: torch.Tensor, audio_sizes: torch.Tensor, device: torch.device):
    # audio_features_proj: (merged_N_tokens, C = 3072)
    audio_features_proj = torch.empty(0, 3072).to(device)
    for i in range(len(audio_sizes)):
        t = audio_set_tensor[i, :audio_sizes[i], :]
        audio_features_proj = torch.cat([audio_features_proj, t], dim=0)
    return audio_features_proj


@torch.jit.script_if_tracing
def calculate_positions(input_ids: torch.LongTensor, features: torch.FloatTensor, special_token_id: int):
    # Calculate positions for image/audio tokens
    if features.numel():
        return torch.where(input_ids == special_token_id)
    return torch.where(torch.zeros((1, 1), dtype=torch.bool))


@torch.jit.script_if_tracing
def select_logic(hidden_states: torch.FloatTensor, features: torch.FloatTensor, positions: List[torch.LongTensor]):
    if features.numel():
        # apply 'select' logic
        hidden_states = hidden_states.index_put(
            positions, features, accumulate=False
        )
...
class Phi4MMEmbedding(nn.Module):
    ...
    def forward(
        ...
        # Mask input ids for image and audio tokens
        ...
        # Calculate position masks
        image_position_mask = input_ids == _IMAGE_SPECIAL_TOKEN_ID
        non_image_position_mask = ~image_position_mask

        # Calculate base hidden states
        hidden_states = self.wte(input_ids)

        # Calculate hidden states for image tokens
        image_positions = calculate_positions(input_ids, image_features, _IMAGE_SPECIAL_TOKEN_ID)
        image_hidden_states = select_logic(hidden_states, image_features, image_positions)

        # Calculate hidden states for audio tokens
        audio_positions = calculate_positions(input_ids, audio_features, _AUDIO_SPECIAL_TOKEN_ID)
        audio_hidden_states = select_logic(hidden_states, audio_features, audio_positions)

        # Merge image, audio, and text hidden states into final hidden states for language model
...
class Phi4MMImageEmbedding(nn.Module):
    ...
    # ONNX forward pass:
    def forward(self, pixel_values: torch.FloatTensor, attention_mask: torch.FloatTensor, image_sizes: torch.LongTensor) -> torch.FloatTensor:
        ...
        if isinstance(self.img_projection, nn.Sequential):
            ...
            target_device = self.img_projection.bias.device
            target_dtype = self.img_projection.bias.dtype
        assert pixel_values.ndim == 5, f'(branch 1) pixel_values size: {pixel_values.size()}, expect 5D tensor for hd transform'

        # Compute image features: Nx(HW)xC
        image_features = self.get_img_features(pixel_values.flatten(0, 1), attention_mask=attention_mask.type(torch.BoolTensor).flatten(0, 1).to(target_device))

        # Calculate height and width of base feature
        base_feat_height = base_feat_width = int(torch.sqrt(image_features.shape[1]))
        assert base_feat_height == self.base_feat_height_target and base_feat_width == self.base_feat_height_target, f'base_feat_height: {base_feat_height}, base_feat_width: {base_feat_width}, expect {self.base_feat_height_target} features for hd transform'

        # bs x max_num_crops x (bfh*bfw) x C
        bs = pixel_values.shape[0]
        image_features = image_features.view(bs, -1, base_feat_height * base_feat_width, self.image_dim_out)

        # all_image_embeddings: (num_img_tokens, 1152)
        all_image_embeddings = get_image_embeddings(
            image_features, attention_mask, image_sizes, self.sub_GN, self.glb_GN,
            bfht=self.base_feat_height_target, crop_size=self.crop_size, bfhr=self.base_feat_height_reduction,
            bfh=base_feat_height, bfw=base_feat_width, C=self.image_dim_out, H=base_feat_height,
...
class Phi4MMAudioEmbedding(nn.Module):
    ...
    def get_audio_features(self, input_embeds: torch.FloatTensor, audio_attention_mask: torch.LongTensor, audio_projection_mode: torch.LongTensor):
    ...
    def forward(self, audio_embeds: torch.FloatTensor, attention_mask: torch.LongTensor, audio_sizes: torch.LongTensor, audio_projection_mode: torch.LongTensor) -> torch.FloatTensor:
        # audio_embeds: (batch_size, num_frames, feature_size)
        # audio_attention_mask: (batch_size, feature_size)
        # audio_sizes: (batch_size)
        # audio_projection_mode: (1) - InputMode.SPEECH or InputMode.VISION
        if isinstance(self.audio_projection, nn.Sequential):  
            target_device = self.audio_projection[0].bias.device  
            target_dtype = self.audio_projection[0].bias.dtype
        elif isinstance(self.audio_projection, nn.ModuleDict):
            target_device = self.audio_projection['speech'][0].bias.device
            target_dtype = self.audio_projection['speech'][0].bias.dtype
        else:  # It's a single nn.Linear layer
            target_device = self.audio_projection.bias.device  
            target_dtype = self.audio_projection.bias.dtype

        # audio_set_tensor: (N_audios, N_padded_tokens, C = 3072)
        audio_set_tensor = self.get_audio_features(audio_embeds, attention_mask, audio_projection_mode)

        # mask: (N_audios, N_padded_tokens)
        # audio_sizes: (N_audios, 1)
        # Create a mask to select the valid audio features
        mask = torch.arange(audio_set_tensor.size(1)).expand(audio_sizes.size(0), -1).to(target_device) < audio_sizes.unsqueeze(1)
        
        # audio_features_proj: (merged_N_tokens, C = 3072)
        # Use the mask to select and concatenate the valid audio features
        audio_features_proj = audio_set_tensor[mask].view(-1, audio_set_tensor.size(2))
...
class Phi4MMForCausalLM(Phi4MMPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if input_mode in [InputMode.VISION_SPEECH, InputMode.VISION]:
            ...
            audio_projection_mode = InputMode.VISION  # 'vision'
        elif input_mode == InputMode.SPEECH:
            self.set_lora_adapter('speech')
            audio_projection_mode = InputMode.SPEECH  # 'speech'
